...
namespace TDJS_Vision.Node._5_EquipmentCommunication.PanasonicWirte
{
    internal class NodePlcWrite : NodeBase
    {
        ...
        /// 节点运行
        /// </summary>
        public override async Task<NodeReturnFlag> Run(CancellationToken token, bool showLog)
        {
            ...
            try
            {
                ...
                if (param.DataType == typeof(bool).Name)
                {
                    ...
                    else
                    {
                        if(param.Value == "1" || param.Value.Equals("True", StringComparison.OrdinalIgnoreCase))
                            res = await param.Plc.WriteBoolAsync(param.Address, true);
                        else
                            res = await param.Plc.WriteBoolAsync(param.Address, false);
                    }
                }
                ...
                else if (param.DataType == typeof(float).Name)
                {
                    ...
                    if (param.Value.Contains("-"))
                    {
                        throw new NotImplementedException(); // 尚未实现写入多个float值
                    }
                    else
                ...
                return NodeReturnFlag.ContinueRun;
            }
